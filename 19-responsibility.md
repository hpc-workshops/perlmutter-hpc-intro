...
best way to transfer them to Perlmutter?

1. `scp -r data user@saul.nersc.gov:~/`
2. `rsync -ra data user@saul.nersc.gov:~/`
3. `rsync -raz data user@saul.nersc.gov:~/`
4. `tar -cvf data.tar data;`

   `rsync -raz data.tar user@saul.nersc.gov:~/`
5. `tar -cvzf data.tar.gz data;`

   `rsync -ra data.tar.gz user@saul.nersc.gov:~/`
